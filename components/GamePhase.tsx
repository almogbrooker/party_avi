...
import { Player, Mission, QAPair, GameState, GameStage } from '../types';
import confetti from 'canvas-confetti';
import { Play, Pause, CheckCircle, XCircle, Wine, Skull, ChevronRight, Volume2, VolumeX, Music, Clock, ThumbsUp, ThumbsDown, User, Crown, Sword, Loader2, ArrowRight } from 'lucide-react';
...
// Roulette spin sound (local, served from public/)
const ROULETTE_SPIN_URL = "/sounds/roulette-wheel.mp3";
const ROULETTE_SPIN_URL_FALLBACK = "https://cdn.pixabay.com/download/audio/2022/03/09/audio_9e1a1261ad.mp3?filename=casino-roulette-110188.mp3";
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
            <img
                ...
                className="w-[70vw] max-w-[480px] h-auto object-contain rounded-3xl border-4 border-yellow-400 shadow-2xl bg-white/10 backdrop-blur-sm animate-fade-in"
            />
        </div>
    );
};
...
const GamePhase: React.FC<GamePhaseProps> = ({ gameState, onUpdateState, onGameEnd }) => {
    ...
    const rouletteSpinRef = useRef<HTMLAudioElement | null>(null);
    ...
    // Intro video (YouTube) configuration
    const INTRO_VIDEO_ID = "e-QFj59PON4";
    const INTRO_START = 45;
    const INTRO_END = 65;

    // Sync roulette spinning with game state
    useEffect(() => {
        if (gameState.isRouletteSpinning !== isRouletteSpinning) {
            setIsRouletteSpinning(gameState.isRouletteSpinning);
        }
    }, [gameState.isRouletteSpinning]);

    // Handle roulette animation with easing (fast start, slow finish)
    useEffect(() => {
        if (!isRouletteSpinning) return;

        // Build the roulette list based on context
        const rouletteIds = gameState.everyoneCorrect
            ? players.filter(p => !p.isGroom).map(p => p.id)
            : roundLosers;
        if (rouletteIds.length === 0) return;

        // Start spin sound loop (light background spin vibe) with fallback if primary fails
        const playSpin = (url: string, triedFallback = false) => {
            if (!isMusicOn) return;
            if (!rouletteSpinRef.current) {
                rouletteSpinRef.current = new Audio(url);
                rouletteSpinRef.current.loop = true;
                rouletteSpinRef.current.volume = 0.35;
                rouletteSpinRef.current.addEventListener('error', () => {
                    if (!triedFallback) {
                        playSpin(ROULETTE_SPIN_URL_FALLBACK, true);
                    }
                });
            } else {
                if (rouletteSpinRef.current.src !== url) {
                    rouletteSpinRef.current.src = url;
                    rouletteSpinRef.current.load();
                }
            }
            rouletteSpinRef.current.play().catch((e) => {
                console.log('Roulette spin play failed:', e);
                if (!triedFallback) {
                    playSpin(ROULETTE_SPIN_URL_FALLBACK, true);
                }
            });
        };
        playSpin(ROULETTE_SPIN_URL);
        ...
        const spinStep = () => {
            ...
            spins += 1;
            // Ease-out: grow delay to slow down over time
            delay = Math.min(60 + spins * 18, 420);
            if (isRouletteSpinning) {
                timer = setTimeout(spinStep, delay);
            }
        };
        ...
        return () => {
            ...
            safePause(rouletteSpinRef);
        };
    }, [isRouletteSpinning, gameState.everyoneCorrect, roundLosers, players]);
    ...
    useEffect(() => {
        ...
        // Create audio objects from uploaded files (no fallbacks - only play if file exists)
        const ensureAudio = (
            ...
            // Check if clip itself is a File or if it has a file property
            const file = clip instanceof File ? clip : clip?.file;
            if (!ref.current && file) {
                console.log(`🎵 Creating ${label} audio from ${file.name}`);
                ref.current = new Audio(URL.createObjectURL(file));
                ref.current.loop = loop;
                ref.current.volume = Number(volume) || 0.5;
            }
        };

        ensureAudio(lobbyMusicRef, gameMusic.lobby, true, 0.5, 'lobby');
        ensureAudio(questionMusicRef, gameMusic.question, true, 0.4, 'question');
        ensureAudio(groomMusicRef, gameMusic.groomAnswering, true, 0.5, 'groom answering');
        ensureAudio(votingMusicRef, gameMusic.voting, true, 0.4, 'voting');
        ensureAudio(revealMusicRef, gameMusic.reveal, false, 0.6, 'reveal');
        ensureAudio(missionMusicRef, gameMusic.mission, true, 0.5, 'mission');
        ensureAudio(victoryMusicRef, gameMusic.victory, false, 0.7, 'victory');

        // Play appropriate music based on phase
        const playPhaseMusic = (audioRef: React.MutableRefObject<HTMLAudioElement | null>, phaseName?: string) => {
            if (audioRef.current) {
                audioRef.current.currentTime = 0;
                // Ensure it's not paused before playing
                if (audioRef.current.paused) {
                    console.log(`🎵 Playing ${phaseName || 'music'}`);
                    audioRef.current.play().catch(err => {
                        console.log('Audio play failed (likely autoplay policy), retrying on user interaction:', err);
                        const resume = () => {
                ...
                console.log(`🎵 No audio ref for ${phaseName || 'music'}`);
            }
        };

        switch (gameState.stage) {
            case GameStage.LOBBY:
                if (lobbyMusicRef.current) playPhaseMusic(lobbyMusicRef, 'lobby music');
                break;
          case GameStage.PLAYING:
              switch (roundPhase) {
                  case 'QUESTION':
                      if (questionMusicRef.current) playPhaseMusic(questionMusicRef, 'question music');
                      break;
                    case 'GROOM_ANSWERING':
                        if (groomMusicRef.current) playPhaseMusic(groomMusicRef, 'groom answering music');
                        break;
                    case 'VOTING':
                        if (votingMusicRef.current) playPhaseMusic(votingMusicRef, 'voting music');
                        break;
                  case 'REVEAL':
                  case 'JUDGMENT':
                  case 'VICTIM_SELECTION': // play reveal music while choosing victim
                  case 'VICTIM_REVEAL':    // keep reveal vibe when the victim is shown
                      if (revealMusicRef.current) playPhaseMusic(revealMusicRef, 'reveal music');
                      break;
                  case 'MISSION_EXECUTION':
                      if (missionMusicRef.current) playPhaseMusic(missionMusicRef, 'mission music');
                      break;
                  case 'DRINK_BREAK':
                      // Keep current music; optionally reuse mission music if available
                      if (missionMusicRef.current) playPhaseMusic(missionMusicRef, 'break music');
                      break;
              }
              break;
            case GameStage.SUMMARY:
                if (victoryMusicRef.current) playPhaseMusic(victoryMusicRef, 'victory music');
                break;
        }
    ...
    // Auto-advance from intro video to first question (host only)
    useEffect(() => {
        if (roundPhase === 'INTRO' && gameState.isHost) {
            const introDuration = (INTRO_END - INTRO_START) * 1000;
            const timer = setTimeout(() => {
                onUpdateState({ roundPhase: 'QUESTION' });
            }, introDuration + 500); // small buffer
            return () => clearTimeout(timer);
        }
    }, [roundPhase, gameState.isHost, onUpdateState]);
    ...
    useEffect(() => {
        ...
        if (roundPhase === 'VICTIM_SELECTION') {
            // Remove automatic roulette when everyone correct - groom should trigger it manually
            // The groom will click to spin the roulette when everyone is correct

            // Bot automation for regular voting losers
            if (groom?.isBot && roundLosers.length > 0) {
                const timeout = setTimeout(() => {
        ...
        // 4. Bot automation for VICTIM_REVEAL to MISSION_EXECUTION
        if (roundPhase === 'VICTIM_REVEAL' && groom?.isBot && selectedVictimId && !gameState.activeMission) {
            const timeout = setTimeout(() => {
                // Select mission by current question index
                const availableMissions = gameState.missions || [];
                const missionIndex = currentQuestionIndex % availableMissions.length;
                const selectedMission = availableMissions.length > 0
                    ? availableMissions[missionIndex]
                    : null;
            ...
            }, 3000); // 3 seconds to read who the victim is
            return () => clearTimeout(timeout);
        }
    ...
    useEffect(() => {
        ...
        if (roundPhase === 'GROOM_ANSWERING') {
            ...
            if (groomAnswer && timeLeft > 0) {
                ...
                const groomTime = gameState.timerSettings?.groomAnswerTime || 60;
                console.log(`⏰ Starting groom timer at ${groomTime} seconds`);
                setTimeLeft(groomTime);
            }
        } else if (roundPhase === 'VOTING') {
            if (timeLeft === 0) {
                const votingTime = gameState.timerSettings?.playerVotingTime || 20;
                console.log(`⏰ Starting voting timer at ${votingTime} seconds`);
    ...
    // Check if all players have voted and auto-advance
    useEffect(() => {
        if (roundPhase === 'VOTING' && gameState.isHost) {
            const nonGroomPlayers = players.filter(p => !p.isGroom);
            const allVoted = Object.keys(currentVotes).length === nonGroomPlayers.length;

            if (allVoted && timeLeft > 0) {
                console.log('✅ All players have voted! Advancing to REVEAL phase');
                setTimeLeft(0); // Stop the timer
                onUpdateState({ roundPhase: 'REVEAL' });
            }
        }
    }, [currentVotes, players, roundPhase, timeLeft, gameState.isHost]);

    // Handle REVEAL phase - play video from answer start
    useEffect(() => {
        if (roundPhase === 'REVEAL' && videoRef.current && currentQ) {
            console.log('🎬 Entering REVEAL phase, playing video from answer start');
            // Resume video playback from the answer start time
            videoRef.current.currentTime = currentQ.aStart;
            videoRef.current.play().catch(err => {
                console.error('Failed to play video in REVEAL phase:', err);
            });
        }
    }, [roundPhase, currentQ]);
    ...
    useEffect(() => {
        if (roundPhase === 'VICTIM_REVEAL' && roundLosers.length > 1) {
            ...
            // Dramatic start sound when spinning begins
    ...
    const handleJudgment = (isCorrect: boolean) => {
        ...
        // SPECIAL RULE: If everyone voted correctly, the last person to vote loses
        const nonGroomPlayers = players.filter(p => !p.isGroom);
        const allVotedCorrectly = losers.length === 0 && Object.keys(currentVotes).length === nonGroomPlayers.length;

        if (allVotedCorrectly && gameState.voteTimestamps) {
            // Find the last voter
            let lastVoterId = '';
            let lastVoteTime = 0;

            Object.entries(gameState.voteTimestamps).forEach(([playerId, timestamp]) => {
                const voteTime = Number(timestamp) || 0;
                if (!players.find(p => p.id === playerId)?.isGroom && voteTime > lastVoteTime) {
                    lastVoteTime = voteTime;
                    lastVoterId = playerId;
                }
            });

            if (lastVoterId) {
                console.log(`🐌 Everyone was correct! Last voter was ${players.find(p => p.id === lastVoterId)?.name} - groom will choose via roulette`);
            }
        }
        ...
        // Determine next phase - always go to VICTIM_SELECTION (someone always does a mission)
        const nextPhase = 'VICTIM_SELECTION';

        // Check if everyone was correct (for auto-selection of last voter)
        const allCorrect = losers.length === 0 && Object.keys(currentVotes).length === players.filter(p => !p.isGroom).length;
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestionIndex + 1 < questions.length) {
      // Trigger a drink break every 4-5 rounds
      const nextIndex = currentQuestionIndex + 1;
      const shouldDrinkBreak = (nextIndex + 1) % 4 === 0 || (nextIndex + 1) % 5 === 0;
      onUpdateState({
        currentQuestionIndex: currentQuestionIndex + 1,
        roundPhase: shouldDrinkBreak ? 'DRINK_BREAK' : 'QUESTION',
        currentVotes: {},
        groomResult: null,
        roundLosers: [],
        groomAnswer: null,
    ...
    return (
        <div className="h-screen w-full flex flex-col bg-black relative overflow-hidden">
            ...
            {/* --- MAIN CONTENT GRID --- */}
            <div className="flex-1 grid grid-cols-12 gap-0 overflow-hidden relative h-full min-h-0">

                {/* LEFT: Video & Actions */}
                <div className="col-span-10 relative bg-black flex flex-col items-center justify-center min-h-0">
                    {/* Video Player */}
                  ...
                  {/* Intro video overlay (host only) */}
                  {roundPhase === 'INTRO' && (
                      <div className="absolute inset-0 bg-black/90 z-30 flex items-center justify-center">
                          {gameState.isHost ? (
                              <div className="w-full max-w-5xl aspect-video">
                                  <iframe
                                    className="w-full h-full rounded-3xl border-4 border-yellow-500 shadow-2xl"
                                    src={`https://www.youtube.com/embed/${INTRO_VIDEO_ID}?start=${INTRO_START}&end=${INTRO_END}&autoplay=1&controls=1&playsinline=1`}
                                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
                                    allowFullScreen
                                  />
                                  <div className="mt-4 text-center">
                                      <button
                                        onClick={() => onUpdateState({ roundPhase: 'QUESTION' })}
                                        className="bg-yellow-500 hover:bg-yellow-400 text-black font-black px-6 py-3 rounded-full shadow-lg border-2 border-black"
                                      >
                                          דלג לפתיחה
                                      </button>
                                  </div>
                              ...
                              <div className="text-center space-y-4">
                                  ...
                                  <h2 className="text-3xl font-black text-white">המארח מקרין פתיח...</h2>
                                  <p className="text-slate-400">תכף מתחילים</p>
                              </div>
                          )}
                      </div>
                  )}
                  ...
                  {/* 3. Victim Roulette Overlay */}
                        {(roundPhase === 'VICTIM_SELECTION' || roundPhase === 'VICTIM_REVEAL') && (
                            <div className="absolute inset-0 bg-slate-900/95 z-20 flex flex-col items-center justify-center">
                                {gameState.everyoneCorrect && roundPhase === 'VICTIM_SELECTION' ? (
                                    <div className="text-center mb-8">
                                        <h2 className="text-5xl font-black text-yellow-400 uppercase tracking-widest animate-pulse drop-shadow-[0_0_20px_rgba(250,204,21,0.8)]">
                                            כולם צדקו!
                                        </h2>
                                        <p className="text-2xl text-red-500 mt-4 animate-pulse">
                                            {gameState.isRouletteSpinning ? 'הגלגל מסתובב...' : 'מי יהיה הקורבן?'}
                                        </p>
                                    </div>
                                ) : (
                                ...
                                <div className="relative w-[600px] h-[600px]">
                                    {(() => {
                                        // Normalize to a list of player IDs to avoid object keys/duplicates
                                        const rouletteIds = gameState.everyoneCorrect
                                            ? players.filter(p => !p.isGroom).map(p => p.id) // All non-groom players
                                            : roundLosers; // Only losers (array of IDs)

                                        if (rouletteIds.length === 0) return null;

                                        const idToShow = isRouletteSpinning
                                            ? rouletteIds[rouletteIndex % rouletteIds.length]
                                            : selectedVictimId || rouletteIds[0];
                                        const currentPlayer = players.find(pl => pl.id === idToShow);

                                        return (
                                            <>
                                                {/* Central roulette circle (always show a face, also while spinning) */}
                                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-56 h-56 rounded-full border-8 border-red-600 overflow-hidden shadow-[0_0_120px_rgba(220,38,38,0.6)] bg-slate-800 z-10">
                                                    {currentPlayer?.photo ? (
                                                        <img src={currentPlayer.photo} className="w-full h-full object-cover animate-pulse" />
                                                    ) : (
                                                ...
                                                {/* All players/losers circles around */}
                                                {rouletteIds.map((playerId, index) => {
                                                    const angle = (360 / rouletteIds.length) * index;
                                                    const radius = 200;
                                                    const x = Math.cos((angle - 90) * Math.PI / 180) * radius;
                                                    const y = Math.sin((angle - 90) * Math.PI / 180) * radius;
                                                    const currentIndex = rouletteIndex % rouletteIds.length;
                                                    const isSelected = isRouletteSpinning
                                                        ? index === currentIndex
                                                        : playerId === selectedVictimId;
                                                    const player = players.find(p => p.id === playerId);

                                                    return (
                                                        <div
                                                            key={playerId}
                                                            className={`absolute w-36 h-36 rounded-full border-4 overflow-hidden transition-all duration-300 ${
                                                                isSelected
                                                                    ? 'border-red-600 scale-150 shadow-[0_0_100px_rgba(220,38,38,0.9)] z-20'
                                                                    : 'border-slate-600 opacity-70 hover:scale-130 hover:opacity-100'
                                                            }`}
                                                            style={{
                                                                left: `${300 + x}px`,
                                                                top: `${300 + y}px`,
                                                            ...
                                                            onClick={() => {
                                                                // Allow clicking only when not spinning and not in automatic selection mode
                                                                if (!isRouletteSpinning && !gameState.everyoneCorrect) {
                                                                    onUpdateState({
                                                                        selectedVictimId: playerId,
                                                                        roundPhase: 'VICTIM_REVEAL',
                                                                        pastVictims: gameState.pastVictims.includes(playerId) ? gameState.pastVictims : [...gameState.pastVictims, playerId]
                                                                    });
                                                                }
                                                            }}
                                                        >
                                ...
                                {/* Groom's SPIN ROULETTE button when everyone is correct */}
                                {gameState.everyoneCorrect && roundPhase === 'VICTIM_SELECTION' && !isRouletteSpinning && gameState.isHost && (
                                    <div className="absolute bottom-32 left-1/2 -translate-x-1/2 z-30">
                                        <button
                                            onClick={() => {
                                                // Find the last voter to ensure they lose
                                                const lastVoterId = Object.entries(gameState.voteTimestamps)
                                                    .filter(([playerId]) => !players.find(p => p.id === playerId)?.isGroom)
                                                    .reduce((last, [playerId, timestamp]) => {
                                                        const voteTime = Number(timestamp) || 0;
                                                        const lastTime = Number(gameState.voteTimestamps[last]) || 0;
                                                        return voteTime > lastTime ? playerId : last;
                                                    }, '');

                                                onUpdateState({
                                                    isRouletteSpinning: true,
                                                    roundLosers: [lastVoterId] // Set the last voter as the loser
                                                });

                                                // Spin animation and land on last voter
                                                setTimeout(() => {
                                                    const targetIndex = players.filter(p => !p.isGroom).findIndex(p => p.id === lastVoterId);
                                                    const spins = 5 + Math.floor(Math.random() * 3); // 5-7 full rotations
                                                    const finalIndex = spins * players.filter(p => !p.isGroom).length + targetIndex;

                                                    onUpdateState({
                                                        rouletteIndex: finalIndex,
                                                        selectedVictimId: lastVoterId,
                                                        roundPhase: 'VICTIM_REVEAL',
                                                        isRouletteSpinning: false,
                                                        pastVictims: gameState.pastVictims.includes(lastVoterId)
                                                            ? gameState.pastVictims
                                                            : [...gameState.pastVictims, lastVoterId]
                                                    });
                                                }, 4000);
                                            }}
                                            className="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white font-bold py-6 px-12 rounded-full text-2xl shadow-2xl transition-all duration-300 hover:scale-105 border-4 border-red-800 animate-bounce"
                                        >
                                            🎰 סובב את הגלגל
                                        </button>
                                    </div>
                  ...
                  {/* DRINK BREAK OVERLAY */}
                  {roundPhase === 'DRINK_BREAK' && (
                      <div className="absolute inset-0 bg-gradient-to-br from-red-900/80 via-black/90 to-orange-900/80 z-30 flex flex-col items-center justify-center text-center gap-8 p-8">
                          <div className="animate-pulse text-7xl">🍻</div>
                          <h2 className="text-5xl font-black text-white drop-shadow-lg">כולם שותים עכשיו!</h2>
                          <p className="text-xl text-orange-200">לחיי החתן! קחו שלוק גדול לפני השאלה הבאה.</p>
                          {gameState.isHost && (
                              <button
                                onClick={() => onUpdateState({ roundPhase: 'QUESTION' })}
                                className="bg-orange-500 hover:bg-orange-400 text-black font-black text-2xl px-10 py-4 rounded-full shadow-2xl border-4 border-white/20 transition-transform hover:scale-105"
                              >
                                  ממשיכים לשאלה
                              </button>
                          )}
                ...
                <div className="col-span-2 bg-slate-900 border-l border-slate-800 flex flex-col z-20 shadow-2xl h-full min-h-0">
                    {/* Header Status */}
                    <div className="p-6 bg-slate-800 border-b border-slate-700">
                        <div className="flex justify-between items-center mb-4">
                            <span className="bg-gradient-to-r from-purple-600 to-pink-600 px-3 py-1.5 rounded-lg font-mono text-white font-bold text-xs animate-pulse">CODE: {gameState.gameCode}</span>
                            <div className="flex items-center gap-2">
                                {roundPhase === 'VOTING' && (
                                    <div className="flex items-center gap-1 text-blue-400 text-xs font-bold bg-blue-900/30 px-3 py-1.5 rounded-lg border border-blue-500/20 animate-pulse">
                                        <Clock className="w-3 h-3" /> הצבעה בתהליך
                                    </div>
                                )}
                    ...
                    {/* Player Grid */}
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-900 min-h-0">
                        <div className="grid grid-cols-2 gap-3">
                            {players.sort((a, b) => b.score - a.score).map(p => {
                                const vote = currentVotes[p.id];
                                const isVoting = roundPhase === 'VOTING';
                                const isReveal = roundPhase === 'JUDGMENT'; // Only show results after groom confirms
                                ...
                                return (
                                  ...
                                  relative p-3 rounded-lg border flex flex-col items-center transition-all duration-300
                              ...
                              `}>
                                        <div className="relative">
                                            <div className={`w-24 h-24 rounded-full bg-slate-700 overflow-hidden border-2 ${p.isBot ? 'border-slate-500 border-dashed' : 'border-slate-600'}`}>
                                        ...
                                        <div className="mt-2 text-center min-w-0 flex-1">
                                            <div className={`font-bold text-sm ${p.isGroom ? 'text-yellow-400' : 'text-slate-200'} truncate`}>
                                                {p.name}
                                            </div>
